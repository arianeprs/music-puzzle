...
class Game{
	...
	constructor(chosenMusic){
		...
		this.hintLimit = 4; // max number of hints 
		...
		this.puzzlePieces =  this.createPuzzlePieces(); //all audio sprites, in shuffled order
...
// ------------- CONSTRUCTOR FUNCTIONS ---------------
	...
	createHowl(){
	  ...
	  var myHowl = new Howl({
	    ...
	    onend: function(){self.puzzlePieces[0].stahp();}
	  });
...
// ---------------- START THE GAME ------------------
	...
	revealGamePanel(bool){
		...
		var playground = document.getElementById("gameContent"),
			welcomePanel = document.getElementById("welcome-panel"),
			aboutBtn = document.getElementById('bottomText');
...
// ---------------- FINISH THE GAME ----------------
	...
	verifyGameFinished(){
		...
		{
			if (!elt.elementDOM.classList.contains('correct-position')){
				gameCompleted = false; 
			}
		}
		if (gameCompleted)
	...
	finishAnimation(){ 
		...
		var myTimeout = setTimeout(function(){ 
			...
			var intervalFinishScreen = setInterval(function(){
				
				if (spritesToColor.length > 0){
					spritesToColor[0].elementDOM.classList.add("hasBeenPlayed", "correct-position"); 
